...
int AddSection(char *filepath, char *sectionName, DWORD sizeOfSection) 
{
	...
	/////////// go through all the available sections,to see section,already exists 
...
bool AddCode(char *filepath)
{
	...
	//nt->OptionalHeader.AddressOfEntryPoint = last->VirtualAddress;   //0x00000000; //
...
/*
	__asm {
		...
		mov eax, [eax + 0x10];						eax = executable LDR_MODULE entry
		mov   ebx, eax;								ebx = executable LDR_MODULE DllBase
		add   eax, [ebx + 0x3c]; PE header VMA		eax = DllBase + e_lfanew(+0x3c) = PEHeader of executable
		lea eax, [eax + 0x28];						eax = executable AddressOfEntryPoint
		mov ebx, SectionStart
		mov [eax], ebx
